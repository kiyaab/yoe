...
export class DrawsService {
  ...
  async executeFullRoundDraw(roundId: string, adminId: string) {
    const round = await this.prisma.lotteryRound.findUnique({
      where: { id: roundId },
    });
    ...
    const confirmedTickets = await this.prisma.ticket.findMany({
      where: { roundId, status: TicketStatus.CONFIRMED },
      include: { user: true },
      orderBy: { ticketNumber: 'asc' },
    });

    if (confirmedTickets.length < 3) {
      throw new BadRequestException('At least 3 confirmed tickets are required to draw 3 prizes.');
    }

    // Step 1: Draw 1st Prize
    const res1 = await this.executeDraw(
      { roundId, position: WinnerPosition.FIRST_PRIZE },
      adminId
    );

    // Step 2: Draw 2nd Prize
    const res2 = await this.executeDraw(
      { roundId, position: WinnerPosition.SECOND_PRIZE },
      adminId
    );

    // Step 3: Draw 3rd Prize
    const res3 = await this.executeDraw(
      { roundId, position: WinnerPosition.THIRD_PRIZE },
      adminId
    );

    // Fetch full winner records
    ...
    const updatedRound = await this.prisma.lotteryRound.findUnique({
      where: { id: roundId },
    });
    ...
    return {
      round: updatedRound,
      winners,
      eligibleTickets: confirmedTickets.map((t) => t.ticketNumber),
    };
  }
}
